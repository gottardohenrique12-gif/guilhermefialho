...
async function garantirModelos() {
  ...
  // Renomeia os arquivos para o formato esperado
  const renomear = [
    ['tiny_face_detector_model.bin', 'tiny_face_detector_model-shard1'],
    ['face_landmark_68_model.bin',   'face_landmark_68_model-shard1'],
    ['face_recognition_model.bin',   'face_recognition_model-shard1'],
  ];
  
  for (const [de, para] of renomear) {
    const origem  = path.join(MODELS, de);
    const destino = path.join(MODELS, para);
